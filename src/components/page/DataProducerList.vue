	...
	<div>
		<div class="container">
			<div class="handle-box">
				<el-input v-model="query.name" placeholder="数据发布名称" class="handle-input mr10" clearable></el-input>
				<el-input v-model="query.code" placeholder="数据发布代码" class="handle-input mr10" clearable></el-input>
				<el-input v-model="query.tableName" placeholder="数据发布表名称" class="handle-input mr10" clearable></el-input>
				<el-button type="primary" icon="el-icon-search" @click="handleSearch">搜索</el-button>
				<el-button type="success" icon="el-icon-lx-roundadd" @click="handleEdit(-1,null)">发布数据</el-button>
				...
				<el-table-column type="expand">
					<template slot-scope="props">
						<table style="width: 100%">
							...
							<tr>
								<td>{{ props.row.updateParameter }}</td>
							</tr>
							<tr>
								<td>新增参数</td>
							</tr>
							<tr>
								<td>{{ props.row.insertParameter }}</td>
							</tr>
							<tr>
								<td>删除参数</td>
							</tr>
							<tr>
								<td>{{ props.row.deleteParameter }}</td>
							</tr>
						</table>
					</template>
				</el-table-column>
		...
		<el-dialog title="编辑数据表数据发布" :visible.sync="addVisible" width="70%">
		...
		methods: {
			...
			createparameter(){
				...
				let params = {
					...
					insertSql: self.form.insertSql,
					updateSql: self.form.updateSql,
					deleteSql: self.form.deleteSql
				};
				params["loginName"]  = localStorage.getItem('ms_username');
				getHttpGet(url, (params)).then(res => {
					let tmpRes = res;
					if (tmpRes.code == 200) {
						...
						self.form.insertParameter = tmpRes.data.insertParameter;
						self.form.updateParameter = tmpRes.data.updateParameter;
						self.form.deleteParameter = tmpRes.data.deleteParameter;
					} else {
						self.$message.error('请求数据异常！' + tmpRes.code);
					}
				}).catch(function(error) {
